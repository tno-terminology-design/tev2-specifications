...
The third section (called `versions`) in the [SAF](@) specifies the [terminologies](@) that are actively maintained by the [curators](@) of the [scope](@). Each such [terminology](@) is [identified](@) (within that [scope](@)) by a (main) [versiontag](@) and optionally also alternative [versiontags](@). The contents of a [terminology](@) is specified by so-called [term selection instructions](@). The [Terminology Construction page](/docs/spec-syntax/terminology-construction) documents the kinds of [term selection instructions](@) that are available, and how they work in the [term](@) selection process.
...
| `termselection` | Y | List of [term selection instructions](@) that are used to generate (this version of) the [scope's](@) [terminology](@). See [Terminology Construction](/docs/spec-syntax/terminology-construction) for details. |
...
:::info Editor's note
The `from` and `to` dates have been included to (in future) enable one to refer to a specific version of the terminology that was valid at a particular date. This feature needs to be worked out, and will impact [terminology construction](/docs/spec-syntax/terminology-construction), [TermRef specs](/docs/spec-syntax/term-ref-syntax), and various tools.
:::
