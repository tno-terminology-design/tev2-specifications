...
| `syntax` | | n | * | This argument has no value. If present, the syntax of all (YAML) fields in the file is checked against their specifications (see e.g. [SAF specs](docs/spec-files/saf), [terminology construction](/docs/spec-syntax/terminology-construction), [MRG specs](/docs/spec-files/mrg), [Curated Texts](/docs/spec-files/curated-text-file), [TermRefs](/docs/spec-syntax/term-ref-syntax)). |
