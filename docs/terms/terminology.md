...
As a consequence, it is easy [to generate](/docs/40-specs/tools/mrgt) an [MRG](@) for a [terminology](@), as this is basically the collection of the [MRG entries](@) that constitute the [terminology](@) (apart from [some meta-data](/docs/40-specs/files/mrg#structure), of course). And from there, [human readable glossaries](hrg@) can be [derived](/docs/40-specs/tools/hrgt).
...
The [terminology construction section](/docs/40-specs/syntax/terminology-construction) describes how to construct a [terminology](@) by selecting (groups of) [terms](scoped-term@), and manipulating such selections.
